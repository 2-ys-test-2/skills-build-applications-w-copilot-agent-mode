    ...
    <section className="card border-0 shadow-sm">
      <div className="card-body">
        <div className="d-flex justify-content-between align-items-center mb-3">
          ...
          <span className="status-badge">{records.length} teams</span>
        </div>

        {error ? (
          <div className="alert alert-warning mb-0">Unable to load teams: {error}</div>
        ) : (
              ...
              <tbody>
                {records.map((team, index) => (
                  <tr key={team._id ?? `${team.name}-${index}`}>
                    <td>{readText(team.name)}</td>
                    <td>{readText(team.coach ?? team.leader)}</td>
                    <td>{readText(team.memberCount ?? team.members?.length ?? 0)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
